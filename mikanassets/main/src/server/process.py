...
このクラスのインスタンスは state.py の `ctx.server_process` として
1つだけ生成され(シングルトン的に使う)、他のモジュールからは
`from state import ctx` した上で `ctx.server_process.xxx()` という形で利用する。
"""
...
class ServerProcess:
    ...
    def start(self, command: list, cwd: str | Path, char_code: str, logger_func) -> subprocess.Popen:
        """
        サーバープロセスを起動し、標準出力を読み続けるロガースレッドを開始する。

        logger_func: (proc, ret_deque) を受け取る関数(main.py の server_logger を渡す想定)

        shell=True は Windows でのみ有効にする: POSIX では shell=True + リスト引数の場合
        command[0] だけがシェルに渡すコマンド文字列として実行され、残りの要素は
        シェルの位置パラメータ($1, $2...)扱いになり server_args が実質無視されてしまう
        (Windows は list2cmdline() で結合されるため問題なく、.bat 起動にも必要)。
        """
